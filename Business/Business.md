...
* 防止手机号登录时被攻击,可以在登录的手机验证码之前再增加一个图形验证码,即有2个验证码
* 图形验证码由后端返回,且返回的应该是一张图片或base64编码后的图片,防止请求被拦截直接获取到验证码内容
* 图形验证码在登录页面一打开的时候就直接返回到前端,不需要和手机号绑定
* 登录时需要将前端提交的手机号和图形验证码绑定,防止同一个请求被调用多次
* 手机验证码需要加上倒计时
...
* 扫码前,手机端应用是已登录状态,PC端显示一个二维码,等待扫描
  * PC端向服务端发起请求,生成用户登录的二维码,并且把PC端设备信息也传递给服务端
  * 服务端收到请求后生成二维码ID,并将二维码ID与PC端设备信息进行绑定,之后将这个二维码ID返回给PC端
  * PC端收到二维码ID后,生成二维码(二维码中肯定包含了ID)
  * 为了及时知道二维码的状态,PC在展现二维码后要不断的轮询服务端,请求服务端告诉当前二维码的状态及相关信息
* 手机端打开应用,扫描PC端的二维码,扫描后提示"已扫描,请在手机端点击确认"
  * 手机扫描PC端的二维码,通过二维码内容取到其中的二维码ID
  * 调用服务端API将移动端的身份信息与二维码ID一起发送给服务端
  * 服务端接收到后,将身份信息与二维码ID进行绑定,生成临时token,然后返回给手机端,并修改二维码ID状态
  * PC端轮询二维码状态,此时就可以在界面上把二维码状态更新为已扫描
* 用户在手机端点击确认,确认后PC端登录就成功了
  * 手机端接收到临时token后弹出确认登录界面,用户点击确认,手机端携带临时token调用服务端的接口,告诉服务端确认登录
...
* 场景:字段A具有唯一性,且表中数据全为逻辑删除
* 当A删除时,因为逻辑删除,数据仍然在表中,不可只对A进行唯一索引建立.因为新增时,数据库仍然会检查到2条相同记录
* 若对A和逻辑删除字段同时建立唯一索引,新增时没问题,但是删除时会索引冲突
* 若加入另外的字段进行版本控制,虽然可以解决该问题,单浪费了性能
* 解决方案:将唯一字段加上一段UUID或雪花算法字符串
...
* 百万级别的会员数,每个会员每天只有几个请求,一个会员信息在同一个请求中被多次调用,且会员信息较大
* 问题:多次调用查询会员方法,组装信息多,数据库查询多
* 解决:使用本地缓存,1分钟有效
* 上述解决方案的问题:频繁fullgc(信息多,1分钟内都存储于新生代),导致短时间大量请求失败
* 解决:去掉本地缓存,使用线程本地变量
...
* 有时间限制,库存限制
* 下单成功后直接支付,若缺少其他必要信息,等支付完之后再填写
* 抢单和支付有时限,如果在时限内未完成支付,取消订单,恢复库存
* 秒杀产生的订单不会立刻写数据库,要结合缓存使用.当秒杀结束或库存为0时再将缓存写回数据库
