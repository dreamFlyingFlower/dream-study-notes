...
* 从MySQL 5.5版本开始默认使用InnoDB作为引擎,主要分为内存结构和磁盘结构两大部分
...
* 默认情况下双写缓冲区是启用的,要禁用Doublewrite Buffer,可以将innodb_doublewrite设置为0
...
* Binlog文件名默认为`主机名_binlog-序列号`格式,例如localhost_binlog-000001,也可以在配置文件中指定名称,文件记录模式有STATEMENT,ROW和MIXED三种:
