...
* 从对数据操作的类型(读/写)分
  * 读锁(共享锁):针对同一份数据,多个读操作可以同时进行,但是加锁的线程不能写,会报错;其他线程可读,但是写会阻塞
  * 写锁(排他锁):只能被一个事务获取,当前写操作没有完成前,会阻断其他线程所有读写操作
* 从对数据操作的粒度分
  * 表锁(偏读)
  * 行锁(偏写)
...
* 若sesson1对a表设置了读锁,那sesson1就只能读a表,不可更新a表(更新报错),也不可读写其他没有锁的表(读报错).sesson2可以读a表和其他表,若是sesson2要写a表,此时session2会被阻塞,直到sesson1解锁,可以直接写其他表
...
* 在行锁的情况下,若更新时索引失效,如where子句中的自动类型转换,使用函数等都将会导致行锁变成表锁,此时整个表都只能有一个更新事务,其他事务都会被阻塞
...
* `show status like 'innodb_row_lock%'`:可配置和`show profile`使用
