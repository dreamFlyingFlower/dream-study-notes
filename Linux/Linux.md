...
* /etc/passwd:用户信息配置文件,每行定义一个用户账户,此文件对所有用户可读,每行账户包含多个信息,用:隔开,如:
	...
	* 口令:即密码.X表示用户口令被/etc/shadow文件保护
	* 用户标识号:也叫UID,系统内唯一.root用户为0,普通用户从500或1000开始,其他的是系统标准账户
...
* /etc/default/useradd:用户组缺省配置文件
  ...
  * SHELL:缺省的解释器
  * SKEL:新添加用户的默认配置文件位置
  * CREATE_MAIL_SPOOL:添加用户后是否为该用户创建爱一个邮件目录

* /etc/group:用户组信息配置文件.该文件对所有用户可读,但组成员做了处理,不可见.多配置:隔开,如:
...
* man cmd:查看某个命令的详情,参数等
* cmd --help:同man
* which cmd:查看命令存在的目录,从环境变量开始查找
* whatis cmd:查看命令的描述
...
* $PATH:系统环境变量
* $UID:查看当前运行命令对象的uid.root的uid是0,其他都大于0
* $PWD:当前所在地址的绝对路径
* $USER:当前登录的用户名
* $HOME:当前登录用户的根目录
* $IFS:默认分割符,在awk等里面会用到,包括空格,换行,制表符
* $LANG:系统语言,编码
* set:查看所有变量,包括环境变量和临时变量
...
* free []:显示当前内存和交换空间的使用情况,单位为k.当内存的free变少时,真正需要看的是Swap中的used信息.Swap分区是由硬盘提供的交换区,当物理内存不够用的时候,操作系统才会把暂时不用的数据放到Swap中.所以当这个数值变高的时候,说明内存真的不够用
  * -h:显示计算单位,同时以最优方式显示数据
...
* top []:显示当前系统中耗资源使用情况,实时
  ...
  * 结果第一行:
    ...
    * up:机器运行了多少时间
    * users:当前有多少用户
    * load average:1,5,15分钟的负载统计值,CPU正在处理以及等待处理的进程数之和
  * 结果第二行:
    * Tasks:当前有多少进程
    * running:正在运行的进程
    * sleeping:正在休眠的进程
...
* `ps []`/`ps[] | more`:显示瞬间的进程状态,通常使用ps -ef或pa -aux
  ...
  * 各项字段含义
    ...
    * PID:进程ID
    * PPID:父进程,0表示无父进程
    * C:进程占用的CPU的百分比.值越大,表明进程是cpu密集型,执行优先级会降低;值越小,表明进程是I/O密集型,执行优先级会提高
    * MEM:进程占用的内存百分比
    * VSZ:进程占用的虚拟内存,单位kb
    * RSS:使用了多少物理内存,单位kb
    * TTY:使用的终端
...
* who/w:显示在线登录用户详情
  ...
  * JCPU:当前用户执行的所有进程耗时总和

  * PCPU:执行程序耗费的时间

  * WHAT:当前执行的什么任务

  * load average:平均负载信息.有3个值,表示最近1分钟,5分钟,15分钟的平均负载.这3个值在0.6和0.7之间是比较标准的,大于0.7就需要优化
...
* `find [path...] [cnd] [action]`:从指定目录以及子目录中查找指定的文件或目录
* `path`:开始查找的目录,若不写,从当前目录查找.可以写多个,用空格隔开
* `cnd`:过滤条件
  * `-a/-o`:and和or的缩写,连接多个条件
  * `-name filename`:根据文件或目录的名称查找.可用通配符,最好加上`""`:`*`多个字符模糊查找,`?`单个占位符查找
  * `-iname filename`:同name,但是忽略大小写
  * `-size 2M/+2M/-2M`:查找大小等于/大于/小于2M的文件
  * `-size +2M -a -size -5M`:查找大于2M,小于5M的文件
  * `-user username`:根据文件所属用户查找
  * `-perm 777`:根据权限查找
  * `-type b/f/d/l`:查找指定文件类型.b二进制文件,f表示文件,d表示目录,l表示软链接
  * `-mtime -20`:查找最近20天内更新过内容的文件
  * `-mtime +20`:查找20天以前更新过内容的文件
  * `-atime n`:查看n天之前访问的文件
...
* `action`:找到文件之后的处理方式.注意,处理方式不是使用的系统的命令,而是find命令自带,要查看帮助手册
  * `-delete`:删除
  * `-exec cmd \;`:执行命令,分号必须带上,`\`转义
  * `-exec cmd {} + \;`:类似于上一个命令,但是会从子文件目录执行.{}表示find命令结果
  * `-exec cmd {} \;`:将find查找的结果用cmd命令再执行一次
...
* useradd/adduser [] username:添加用户
  * -u uid:设置uid,一般不指定
  * -p passwd:设置密码
  * -g groupname:设置分组,若分组不存在,报错.若不指定,默认创建一个同名的用户组,并将用户归到新建的用户组中
  * -G groupname1 groupname2:将用户同时指定到多个其他附加组
  * -M:创建一个没有主目录的用户,类似于mysql之类,无法正常登陆linux
  * -s shellcmd:设置shell命令
    * useradd test -s /sbin/nologin:新建一个不可登陆的账户test.可以参考mysql帐号
  * -N:只创建用户,不创建组
  * -r:创建系统用户
...
* gpasswd [] username groupname:用户组操作
  * -a:将指定用户添加到指定组
  * -d:将用户从指定组中删除
  * -A:将用户指定为组的管理员
...
> 需要按:(冒号)进入到命令模式,一般保存,搜索等操作需要进入该模式
