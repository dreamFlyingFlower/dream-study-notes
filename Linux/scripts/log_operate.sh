...
# 可以使用如下参数查看数据,统计httpd进程数.连个请求会启动一个进程,使用于Apache服务器,表示Apache能够处理n个并发请求,这个值Apache可根据负载情况自动调整
...
# 关闭一个TCP连接需要从两个方向上分别进行关闭,双方都是通过发送FIN来表示单方向数据的关闭,
# 当通信双方发送了最后一个FIN的时候,发送方此时处于LAST_ACK状态,
# 当发送方收到对方的确认（Fin的Ack确认）后才真正关闭整个TCP连接
LAST_ACK 5
# 表示正在等待处理的请求数
SYN_RECV 30
# 表示正常数据传输状态
