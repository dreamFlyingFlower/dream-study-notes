...
########				Java程序启动脚本
########       	1.上传脚本到服务器执行目录
########       	2.赋权:命令为chmod 755 server_java.sh
########				3.若APP_NAME为相对路径,需要根据路径移动当前脚本
########				4.若APP_NAME为绝对路径,可放在任意目录
########				5.执行命令./server_java.sh start或sh server_java.sh start
#####################################################################

# 遇到错误立即退出
set -e
# JDK地址
JDK_HOME="/usr/local/java/jdk1.8.0_311/bin/java"
# VM选项,如-Xms256m -Xmx512m -Dspring.config.additional-location=./app/config/
VM_OPTS=" -Xms256m -Xmx256m "
# program arguments,程序参数,如 --spring.profiles.active=dev
SPB_OPTS=" "
# Jar包地址
APP_NAME="./app/srt-cloud-data-governance-2.0.0.jar"
# Jar运行PID
...
restart() {
  echo "=============================restart stop=============================="
  stop
  echo "===========================restart stop end============================"
  sleep 1
  echo "============================restart start=============================="
  start
  echo "===========================restart start end============================"
}
