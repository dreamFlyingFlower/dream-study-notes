...
* 当 Acceptor 处理完客户端连接事件之后(与客户端建立好 Socket 连接),MainReactor 将 连接分配给 SubReactor.即MainReactor 只负责监听客户端连接请求,和客户端建立连接之后将连接交由 SubReactor 监听后面的 IO 事件
...
* 这种模式也被叫做服务器的 1+M+N 线程模式,即使用该模式开发的服务器包含一个或多个连接建立线程+M 个 IO 线程+N 个业务处理线程